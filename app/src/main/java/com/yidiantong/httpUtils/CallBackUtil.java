...
public abstract class CallBackUtil<T> {
    ...
    public void onSuccess(Call call, Response response) {
        ...
        mMainHandler.post(new Runnable() {
            ...
            public void run() {
                // token过期时，发送广播
                if (obj != null) {
                    LogUtils.info("CallBackUtil", "response.body = " + obj.toString());
                    if (obj.toString().contains("Token无效或已过期")
                            || obj.toString().contains("Token is invalid")) {
                        EventBus.getDefault().post(Constants.TOKEN_TIMEOUT);
                    }
                }
                onResponse(obj);
            }
    ...
    public static abstract class CallBackString extends CallBackUtil<String> {
        ...
        public CallBackString(Context mContext) {
