...
public class ApiRequestMaker {
    public static void make(ApiRequest request, String appKey, String appSecret) {
        ...
        if (request.getToken() != null && !request.getToken().equals("")) {
            request.addHeader(HttpConstant.TOKEN, request.getToken());
        }
        ...
        // add "X-Ca-Signature-Method" if need @归空 @ Begin
        if (!TextUtils.isEmpty(request.getSignatureMethod())) {
        ...
        if (null != request.getBody() && request.getBody().length > 0) {
            request.addHeader(HttpConstant.CLOUDAPI_HTTP_HEADER_CONTENT_MD5, base64AndMD5(request.getBody()));
        }
         ...
         */
        request.addHeader(SdkConstant.CLOUDAPI_X_CA_SIGNATURE, SignUtil.sign(request, appSecret));
    ...
    private static String base64AndMD5(byte[] bytes) {
        ...
        try {
            ...
            return new String(encodeBytes2, SdkConstant.CLOUDAPI_ENCODING);
